...
/** packet type change when psm value is obex */
	 if (l2cap_pi(sk)->psm == cpu_to_le16(0x0003))
		 packet_type = 0x770e;
	 else
		 packet_type = 0;

	 hcon = hci_connect(hdev, ACL_LINK, packet_type , dst,
...
/*	if(flags & ACL_START) {
		...
		*/
		if (conn->rx_len) {
			printk("Unexpected start frame (len %d), (conn->rx_len %d)\n", skb->len, conn->rx_len);
			kfree_skb(conn->rx_skb);
			conn->rx_skb = NULL;
			conn->rx_len = 0;
		...
		printk("ulbbang - Start: total len %d, frag len %d\n", len, skb->len);
		if (len == skb->len) {
		...
		printk("ulbbang - Cont: frag len %d (expecting %d)\n", skb->len, conn->rx_len);
