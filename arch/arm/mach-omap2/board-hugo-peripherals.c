...
/**
 * File Name : board-hugo-peripherals.c
 *
 * File Description :
 *
...
#ifdef HUGO_CAM	// For cam driver 29.11.2011 (jinokang)
#include "../../../drivers/media/video/s5k5bbgx.h"
#include "../../../drivers/media/video/sr030pc30.h"
...
struct s5k5bbgx_platform_data omap_board_s5k5bbgx_platform_data;
struct sr030pc30_platform_data omap_board_sr030pc30_platform_data;
//struct sr200pc10_platform_data omap_board_s5k4ca_platform_data;
#else
struct ce147_platform_data omap_board_ce147_platform_data;
...
#if ( defined(CONFIG_MPU_SENSORS_MPU6050B1_411) && ( CONFIG_SAMSUNG_REL_HW_REV >= 2 ) )
...
#define FUEL_I2C_SDA  61	
#define FUEL_I2C_SCL  60

#ifdef CONFIG_WILINK_NLCP 
#include <linux/wl12xx.h> 
#define WIFI_EN_GPIO		160
#define WIFI_IRQ_GPIO		99
#endif
...
#endif //CONFIG_MPU_SENSORS_MPU6050B1_411
...
#if defined( CONFIG_MACH_SAMSUNG_HUGO )
...
struct regulator_init_data omap_board_vdac = {
	...
	.num_consumer_supplies = ARRAY_SIZE(omap_board_vdda_dac_supply),
...
static struct regulator_init_data omap_board_vmmc1 = {
	.constraints = {
		.min_uV			= 3000000, //1850000,
		.max_uV			= 3000000, //3150000,
		.valid_modes_mask	= REGULATOR_MODE_NORMAL | REGULATOR_MODE_STANDBY,
		.valid_ops_mask		= REGULATOR_CHANGE_MODE | REGULATOR_CHANGE_STATUS,
	},
	.num_consumer_supplies  = ARRAY_SIZE(omap_board_vmmc1_supply),
...
static struct regulator_init_data omap_board_vmmc2 = {
	...
	.num_consumer_supplies  = ARRAY_SIZE(omap_board_vmmc2_supply),
...
static struct regulator_init_data omap_board_aux1 = {
	...
	.num_consumer_supplies = ARRAY_SIZE(omap_board_vaux1_supply),
...
static struct regulator_init_data omap_board_aux2 = {
	...
	.num_consumer_supplies = ARRAY_SIZE(omap_board_vaux2_supply),
...
static struct regulator_init_data omap_board_vsim = {
	...
	.num_consumer_supplies  = ARRAY_SIZE(omap_board_vsim_supply),
...
static struct regulator_init_data omap_board_vpll2 = {
	...
	.num_consumer_supplies  = ARRAY_SIZE(omap_board_vpll2_supply),
...
static struct regulator_init_data omap_board_aux3 = {
	...
	.num_consumer_supplies = ARRAY_SIZE(omap_board_vaux3_supply),
...
static struct regulator_init_data omap_board_aux4 = {
	...
	.num_consumer_supplies = ARRAY_SIZE(omap_board_vaux4_supply),
...
static struct __initdata twl4030_power_data hugo_t2scripts_data;

#ifdef CONFIG_WILINK_NLCP
static struct wl12xx_platform_data hugo_nlcp_data __initdata = {
	.irq = OMAP_GPIO_IRQ(WIFI_IRQ_GPIO),
	.board_ref_clock = WL12XX_REFCLOCK_26,
	.board_tcxo_clock = 1,
	.platform_quirks = 1,
};

static void hugo_wlan_data_init(void)
{
	printk("*******hugo_wlan_data_init() *****\n");
	
	if (wl12xx_set_platform_data(&hugo_nlcp_data))
		...
		unsigned gpio, unsigned ngpio)
{
	...
	*/
	...
	omap_board_vmmc1_supply[0].dev = mmc[0].dev;
	omap_board_vmmc1_supply[1].dev = mmc[1].dev;
	omap_board_vsim_supply[0].dev = mmc[2].dev;
...
#ifdef CONFIG_KEYBOARD_HUGO_GPIO_KEY

#define HUGO_GPIO_KEY_DEBOUNCE_INTERVAL 10

static struct gpio_keys_button hugo_gpio_keys_button[] = {
	/*
	{
		.desc			= "PWR_KEY",
		.type			= EV_KEY,
		.code			= KEY_POWER,
		.gpio			= OMAP_GPIO_KEY_HUGO_PWRON,
		.active_low		= 1, 
		.debounce_interval	= HUGO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	{
		.desc			= "HOME_KEY",
		.type			= EV_KEY,
		.code			= KEY_HOME,
		.gpio			= OMAP_GPIO_KEY_HUGO_HOME,
		.active_low		= 1, 
		.debounce_interval	= HUGO_GPIO_KEY_DEBOUNCE_INTERVAL,
	},
	*/
...
static struct gpio_keys_platform_data hugo_gpio_keys_platform_data = {
	.buttons = hugo_gpio_keys_button,
	.nbuttons = ARRAY_SIZE(hugo_gpio_keys_button),
	//.rep = 1;
};

static struct platform_device hugo_gpio_key_device = {
	.name = "sec_key",
	.id = -1,
	.dev = {
		.platform_data = &hugo_gpio_keys_platform_data,
	},
};
#endif
...
static struct twl4030_clock_init_data omap_board_clock_data = {
	.ck32k_lowpwr_enable = true,
};
...
static struct i2c_board_info __initdata board_i2c_boardinfo1[] = {
...
#elif defined(CONFIG_MAX14577_MICROUSB)
	{
		I2C_BOARD_INFO("max14577", 0x25),
		.flags = I2C_CLIENT_WAKE,
		.irq = OMAP_GPIO_IRQ(OMAP_GPIO_JACK_NINT),
	},
...
#ifdef HUGO_CAM
	{
		I2C_BOARD_INFO(S5K5BBGX_DRIVER_NAME, S5K5BBGX_I2C_ADDR),
		.platform_data = &omap_board_s5k5bbgx_platform_data,
	},
	{
		I2C_BOARD_INFO(SR030PC30_DRIVER_NAME, SR030PC30_I2C_ADDR),
		.platform_data = &omap_board_sr030pc30_platform_data,
	},
#else
	{
	...
	{
		I2C_BOARD_INFO(SR200PC10_DRIVER_NAME, SR200PC10_I2C_ADDR),
		.platform_data = &omap_board_s5k4ca_platform_data,
	},
#endif
	...
	.irq = OMAP_GPIO_IRQ(OMAP_GPIO_GYRO_INT),
	.platform_data = &mpu6050_data,
	},
#if ( defined(CONFIG_MPU_SENSORS_YAS530_411) && ( CONFIG_SAMSUNG_REL_HW_REV >= 2 ) )
	{
	I2C_BOARD_INFO("yas530", 0x2E),
	.platform_data = &inv_mpu_yas530_data,
	},
#endif //CONFIG_MPU_SENSORS_YAS530_411
#endif //CONFIG_MPU_SENSORS_MPU6050B1_411

#if ( defined(CONFIG_INPUT_YAS_MAGNETOMETER_411) && ( CONFIG_SAMSUNG_REL_HW_REV >= 2 ) )
	{
	I2C_BOARD_INFO("geomagnetic", 0x2e),
	},
#endif		
...
static struct i2c_board_info __initdata board_i2c_boardinfo3[] = {
#ifdef CONFIG_TOUCHSCREEN_MELFAS
    {
        I2C_BOARD_INFO(MELFAS_TS_NAME, 0x48),// 10010(A1)(A0)  A1=PD0, A0=M(0=12bit, 1=8bit)
	...
	{
		I2C_BOARD_INFO("sec_touchscreen", (0x40>>1)),
	},
#endif	
};
...
static inline void __init board_init_battery(void)
{
	...
	gpio_direction_input(OMAP_GPIO_JACK_NINT);
#else
	if (gpio_request(OMAP_GPIO_USBSW_NINT, "ta_nconnected irq") < 0) {
		printk(KERN_ERR
			   "Failed to request GPIO%d for ta_nconnected IRQ\n",
			   OMAP_GPIO_USBSW_NINT);
		samsung_charger_resources[0].start = -1;
	} else {
		samsung_charger_resources[0].start =
			gpio_to_irq(OMAP_GPIO_USBSW_NINT);
	}
#endif
	if (gpio_request(OMAP_GPIO_VF, "VF irq") < 0) {
		printk(KERN_ERR "Failed to request GPIO%d for VF IRQ\n", OMAP_GPIO_VF);
		samsung_charger_resources[1].start = -1;
	} else {
		samsung_charger_resources[1].start = gpio_to_irq(OMAP_GPIO_VF); 
	}
#else
...
void __init omap_board_peripherals_init(void)
{
	...
	twl4030_get_scripts(&hugo_t2scripts_data);
