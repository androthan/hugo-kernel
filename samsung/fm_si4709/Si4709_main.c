...
/*ISR*/
irqreturn_t Si4709_isr( int irq, void *unused );
	...
	switch (ioctl_cmd)
	{
        ...
        case Si4709_IOC_SEEK_AUTO:
                 ...
                 else
                 {
                     ...
                     {
                         error("Si4709_IOC_SEEK_AUTO failed");
                     }
		...
		case Si4709_IOC_SYS_CONFIG2_SET:
			{
				...
				if(n)
				{
					error("Si4709_IOC_SYS_CONFIG2_SET() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
				}
		...
		case Si4709_IOC_SYS_CONFIG3_SET:
			{
				...
				if(n < 0)
				{
					error("Si4709_IOC_SYS_CONFIG3_SET() : copy_from_user() has error!!Failed to read [%d] byes!", n);
					ret = -EFAULT;
		...
		case Si4709_IOC_MUTE_GET:	//JinoKang 2011.06.03
			{
				int isMute;
				debug("Si4709_IOC_MUTE_GET called");

				if( (ret = Si4709_dev_mute_get(&isMute)) < 0)
					error("Si4709_IOC_MUTE_GET failed");
				else if( copy_to_user(argp, (void*) &isMute, sizeof(int)) )
					ret = -EFAULT;			
			}  
			break;

#ifdef FMRADIO_TSP_DELAY_CONTROL
		case Si4709_IOC_TSP_SPEED_SET:
			{
				unsigned int tsp_speed;
				debug("Si4709_IOC_TSP_SPEED_SET called");      

				if(copy_from_user((void*) &tsp_speed, argp, sizeof(unsigned int)))
					ret = -EFAULT;
				else if ((ret = Si4709_dev_tsp_speed_set(tsp_speed)) < 0)
					error("Si4709_IOC_TSP_SPEED_SET failed");
			}
			break;
#endif
		...
		/*VNVS:END*/
		default:
			error("ioctl default: cmd = %d", ioctl_cmd);
			ret = -ENOTTY;
			break;
	}
...
irqreturn_t Si4709_isr( int irq, void *unused )
{
	...
	else if(Si4709_RDS_flag == RDS_WAITING) //RDS Interrupt
	{
		...
		debug("RDS_Groups_Available_till_now b/w Power ON/OFF : %d",RDS_Groups_Available_till_now);
...
void debug_ioctls(void) 
{
   ...
   // debug("Si4709_IOC_SEEK_AUTO 0x%x",Si4709_IOC_SEEK_AUTO );
	...
	debug("Si4709_IOC_MUTE_GET 0x%x",Si4709_IOC_MUTE_GET);	//JinoKang 2011.06.03
