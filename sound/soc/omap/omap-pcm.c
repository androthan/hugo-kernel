...
static int omap_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	DEBUG_LOG("1. %s before lock %d\n", __func__, preempt_count());
	spin_lock_irqsave(&prtd->lock, flags);
	DEBUG_LOG("2. %s after lock %d\n", __func__, preempt_count());
#if 0
	switch (cmd) {
	...
	if (cmd == SNDRV_PCM_TRIGGER_START || cmd == SNDRV_PCM_TRIGGER_RESUME || cmd == SNDRV_PCM_TRIGGER_PAUSE_RELEASE) {
		...
		if(substream->stream == SNDRV_PCM_STREAM_CAPTURE){
			...
			u32 ocp_reg = omap_readl(0x4805602C); 		
	...
	DEBUG_LOG("3. %s before unlock %d\n", __func__, preempt_count());
	spin_unlock_irqrestore(&prtd->lock, flags);
	DEBUG_LOG("4. %s after unlock %d\n", __func__, preempt_count());
...
static int omap_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		DEBUG_LOG("AlsaDumpfile is  Opened. Error No = %d\n", file_error);

		if(IS_ERR(pDumpFile)){
			file_error = PTR_ERR(pDumpFile);
			DEBUG_LOG("AlsaDumpfile is Not Opened. Error No = %d\n", file_error);
			goto out;
		}
	}
#endif
...
static int omap_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if(!(IS_ERR(pDumpFile))){
			...
			DEBUG_LOG("AlsaDumpFile is closed\n");
			pDumpFile = NULL;
		}
	}
#endif
