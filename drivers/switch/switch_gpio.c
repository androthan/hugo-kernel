...
#define SEC_HEADSET_DBG(fmt, arg...) printk(KERN_ERR "[HEADSET] %s () " fmt "\r\n", __func__, ## arg)
#else
...
{
	SEC_HEADSET_DBG("[Headset]Headset attached\n");
	headset_status = 1;
...
static int get_t2adc_data( int ch )
{
	...
	SEC_HEADSET_DBG("3pole headset detecing use sendend %d\n", sendend_state);
...
static int get_t2adc_data( int ch )
{
    ...
    ret = req.rbuf[ch];
	SEC_HEADSET_DBG("adc value is : %d\n", ret);
...
extern struct snd_soc_codec *gCodec;
extern void wm1811_set_ear_state(u16 headset_status);
extern void WM1811_Switch_To_Earphone(struct snd_soc_codec *codec, u16 status);
extern void wm1811_ear_micbias_set(struct snd_soc_codec *codec, u16 on);
extern int get_adc_data( int ch );
extern void set_adc_compensation(int adcComp);

int earjack_min_max_adc = 0 ;

const u16 adcCompValue[11] = {18, 18, 16, 14, 12, 10, 8, 6, 4, 3, 2};
...
static void ear_adc_caculrator(struct work_struct *work)
{
	...
	{
#if 0
		//wm1811_ear_micbias_set(gCodec, OFF);		// if state is the 1, ear micbias was always off.
		//msleep(50);
		adcGround = get_adc_data(2);
		SEC_HEADSET_DBG ("!!!!!!!!!!!!!!!!!!!adcGround=%d\n", adcGround);
		adcComp = abs(10 - adcGround);		// on the initial status, 10 is max adc value
		if (adcGround > 10)
		{
			adcComp *= -1;
			SEC_HEADSET_DBG ("!!!!!!!!!!!!!!!!!!!adcComp=%d\n", adcComp);		
		}
		SEC_HEADSET_DBG ("!!!!!!!!!!!!!!!!!!!adcComp=%d\n", adcComp);
		set_adc_compensation(adcComp);
#else
		adcGround = get_adc_data(2);
		SEC_HEADSET_DBG ("!!!!!!!!!!!!!!!!!!!adcGround=%d\n", adcGround);
		if (adcGround > 10)
			...
			adcComp=adcCompValue[adcGround];
		SEC_HEADSET_DBG ("!!!!!!!!!!!!!!!!!!!adcComp=%d\n", adcComp);
		set_adc_compensation(adcComp);
#endif

		wm1811_ear_micbias_set(gCodec, ON);
		msleep(50);		// make time to apparent micbias
		
		#ifdef USE_ADC_SEL_GPIO
		gpio_direction_output(EAR_ADC_SEL_GPIO , 1);
		#endif

		earjack_min_max_adc = get_t2adc_data(2);

		#ifndef USE_ONLY_AS_4POLE
		if (earjack_min_max_adc > ear_4pole_boundary)
		#endif
		{	
			switch_set_state(&data->sdev,HEADSET_4POLE_WITH_MIC);
			headset_status = HEADSET_4POLE_WITH_MIC;

			SEC_HEADSET_DBG("[Headset] 4pole ear-mic adc is %d\n", earjack_min_max_adc);
			#ifdef CONFIG_INPUT_ZEUS_EAR_KEY
			//msleep(2000);

			if (gpio_get_value(EAR_DET_GPIO) ^ EAR_DETECT_INVERT_ENABLE)		{
				SEC_HEADSET_DBG("\t[Headset]OK!!!");
				ear_key_enable_irq();
			}
			else		{
				SEC_HEADSET_DBG("\t[Headset]Return!!!");
				return;
			}
		...
		else 
		{
			...
			wm1811_ear_micbias_set(gCodec, OFF);
			
			SEC_HEADSET_DBG("[Headset] 3pole earphone adc is %d\n", earjack_min_max_adc);
			#ifdef CONFIG_INPUT_ZEUS_EAR_KEY
			ear_key_disable_irq();
			#endif
	...
	{
		SEC_HEADSET_DBG("[Headset] Headset detached\n");        	
		...
		WM1811_Switch_To_Earphone(gCodec, OFF);		
	...
	wm1811_set_ear_state(headset_status);
...
static void headset_detect_timer_handler(unsigned long arg)
{
	...
	{
		SEC_HEADSET_DBG(KERN_ALERT "wrong headset_detect_timer_token count %d", headset_detect_timer_token);
...
static ssize_t switch_gpio_set_state(struct switch_dev *sdev, char *buf)
{
	SEC_HEADSET_DBG("Switch_gpio_set_state() buf : %c\n", buf[0]);
	...
	switch(buf[0])
	{
		...
		case '1' :
			...
			SEC_HEADSET_DBG("[Headset] 4pole ear-mic%d\n");  
			#ifdef CONFIG_INPUT_ZEUS_EAR_KEY
			msleep(2000);
			ear_key_enable_irq();
			#endif
...
{
	/*
	* This feature is not required at the Hugo project.
	if(headset_status == HEADSET_4POLE_WITH_MIC)
	{
		SEC_HEADSET_DBG("[HEADSET] 4POLE_HEADSET enable VINTANA2, USB3V1");
	...
	*/
	...
	if (wake_lock_active(&headset_wake_lock) > 0)
	{
		wake_unlock(&headset_wake_lock);
	}
...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	switch_data->sdev.set_state = switch_gpio_set_state;
