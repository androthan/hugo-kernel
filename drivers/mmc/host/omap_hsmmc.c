...
static int wifi_set_power(struct device *dev, int slot, int power_on, int vdd)
{
	...
	if (power_on) {
		gpio_set_value(WIFI_EN_GPIO, 1);
		mdelay(15);
		gpio_set_value(WIFI_EN_GPIO, 0);
		mdelay(1);
		gpio_set_value(WIFI_EN_GPIO, 1);
		mdelay(70);
	} else {
				   ...
				   int vdd)
{
	 ...
	 */
/* AALTO(taejin.hyeon) 2011.04.25 bring up wifi, it is temp. */
#if 0
	if (power_on) {
		ret = mmc_regulator_set_ocr(host->vcc, vdd);
...
#else
		mmc_slot(host).set_power = wifi_set_power;
#endif
		mmc_slot(host).set_sleep = omap_hsmmc_23_set_sleep;
		break;
