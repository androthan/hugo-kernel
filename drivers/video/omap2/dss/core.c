...
#if (defined(CONFIG_MACH_SAMSUNG_HUGO) && (CONFIG_SAMSUNG_REL_HW_REV < 2))
#include <linux/wakelock.h>	//temp jy1004.park	too prevent suspend lock up

struct wake_lock wake_lock_test;   // declared globally	//temp jy1004.park	too prevent suspend lock up
#endif

static bool dep_set = false;

u32 sgx_sleepdep_value = 0;
u32 dss_sleepdep_value = 0;
int dss_suspend_flag = 0;

int dss_resume_flag = 0;

int  dss_resume_complete = 1;
...
static int omap_dss_suspend(struct platform_device *pdev, pm_message_t state)
{
...
#if (defined(CONFIG_MACH_SAMSUNG_HUGO) && (CONFIG_SAMSUNG_REL_HW_REV < 2))
	 wake_lock(&wake_lock_test);		//jy1004.park	suspend lock up
#endif
	...
	dss_resume_complete = 0;
	ret = dss_suspend_all_devices();

	dss_sleepdep_value = omap_readl(0x48306EC8);


	if(audio_on)
   	{
       printk(" DSS dependency removed\n");
       omap_writel(dss_sleepdep_value & ~(1<<1) , 0x48306EC8); // PM_WKDEP_DSS
	   dep_set = true;
   	}
...
static int omap_dss_resume(struct platform_device *pdev)
{
...
#if (defined(CONFIG_MACH_SAMSUNG_HUGO) && (CONFIG_SAMSUNG_REL_HW_REV < 2))
	wake_unlock(&wake_lock_test);		//jy1004.park		//suspend lock up
#endif

	if(dep_set)
   	{
		printk(" DSS dependency restored\n");
		omap_writel(dss_sleepdep_value, 0x48306EC8);//PM_WKDEP_DSS
		dep_set = false;
   	}
	...
	printk(" *****rtyuiop[*******\n");
	     ...
	     ret = dss_resume_all_devices();
	     printk("--- omap_dss_resume dss ---\n");
     
	dss_resume_flag = 0;
	dss_resume_complete = 1;
...
#if (defined(CONFIG_MACH_SAMSUNG_HUGO) && (CONFIG_SAMSUNG_REL_HW_REV < 2))
	wake_lock_init(&wake_lock_test, WAKE_LOCK_SUSPEND, "test_wakelock");	//jy1004.park	suspend lock up
#endif
