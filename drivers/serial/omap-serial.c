...
struct mutex  gps_uart_wa;
static u32 gps_uart_wa_open_flag = GPS_UART_CLOSE;
...
static int serial_omap_probe(struct platform_device *pdev)
{
	...
	if (omap_up_info->use_dma) {
		...
		up->uart_dma.rx_timeout = omap_up_info->dma_rx_timeout;//2012.01.25 deukmo.lee OMAP Serial UART DMA RX_TIMEOUT BUG FIX
...
#ifdef GPS_UART_WORKAROUND
		mutex_lock(&gps_uart_wa);
		if (num == UART1 && gps_uart_wa_open_flag == GPS_UART_CLOSE)	//uart1 and already closed.
		{
			printk("Clear RX FIFO --> unexpected GPS rx activity\n");
			serial_out(up, UART_FCR, 0x2); //clear RX FIFO
		}			
		mutex_unlock(&gps_uart_wa);
#endif  //GPS_UART_WORKAROUND
