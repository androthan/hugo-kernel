...
static void board_poweroff(void)
{
	...
	printk("%s : microusbic state = %d\n", __func__, n_usbic_state);

#if ( defined( CONFIG_MACH_SAMSUNG_LATONA ) || defined( CONFIG_MACH_SAMSUNG_AALTO) || defined( CONFIG_MACH_SAMSUNG_HUGO) ) // jypark72, to avoid build error
	/* Aalto doesn't use OMAP_GPIO_TA_NCONNECTED. So use usbic state instead... */
	if (gpio_get_value( OMAP_GPIO_JIG_ON18) || 
		n_usbic_state == MICROUSBIC_JIG_UART_ON ||
		n_usbic_state == MICROUSBIC_TA_CHARGER ||
		n_usbic_state == MICROUSBIC_USB_CHARGER || 
...
#endif
	{
		printk("Warmreset by TA or USB or Jtag - JIG_ON18 \n\n");
		printk("Warmreset by TA or USB or JIG - Use microusb ic state \n\n");
