...
ssize_t gpiokey_pressed_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	key_press_status |= !gpio_get_value(OMAP_GPIO_KEY_HUGO_VOLUP);
	key_press_status |= !gpio_get_value(OMAP_GPIO_KEY_HUGO_VOLDN);
#endif
...
#elif defined( CONFIG_MACH_SAMSUNG_HUGO )
MODULE_DESCRIPTION("board hugo power and home key");
#endif
MODULE_LICENSE("GPL");
