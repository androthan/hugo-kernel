 ...
 *****************************************************
 *****************************************************
 * modules/camera/s5k5bbgx_platform.c
 *
 * S5K5BBGX sensor driver file related to platform
 *
...
#define S5K5BBGX_BIGGEST_FRAME_BYTE_SIZE  PAGE_ALIGN(1280 * 720 * 2 *6) //fix for usage of 6 buffers for 720p capture and avoiding camera launch issues.

static struct v4l2_ifparm ifparm_s5k5bbgx = {
  ...
  .u = {
    .bt656 = {
      ...
      .clock_min = S5K5BBGX_XCLK,
      .clock_max = S5K5BBGX_XCLK,
      .clock_curr = S5K5BBGX_XCLK,
    },
  },
};

static struct omap34xxcam_sensor_config s5k5bbgx_hwc = {
  .sensor_isp = 1,
  //.xclk = OMAP34XXCAM_XCLK_A,
  .capture_mem =  S5K5BBGX_BIGGEST_FRAME_BYTE_SIZE, 
};

struct isp_interface_config s5k5bbgx_if_config = {
...
static int s5k5bbgx_enable_gpio(void)
{
#if 1 //LYG
  printk(S5K5BBGX_MOD_NAME "s5k5bbgx_enable_gpio is called...\n");
  
  /* Request and configure gpio pins */
  if (gpio_request(OMAP3430_GPIO_CAMERA_EN,"CAM EN") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN);
    //return -EIO;
  } 
  if (gpio_request(OMAP3430_GPIO_CAMERA_EN2,"CAM EN2") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN2);
    //return -EIO;
  }
  if (gpio_request(OMAP3430_GPIO_CAMERA_EN3,"CAM EN3") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN3);
   // return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_EN4,"CAM EN4") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_EN4);
   // return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_STBY,"VGA STDBY") != 0)
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_STBY);
   // return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_RST,"CAM VGA RST") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_RST);
   // return -EIO;
  } 
  ...
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_SDA);
    //return -EIO;
  }

  if (gpio_request(OMAP3430_GPIO_CAMERA_SCL,"CAM VGA RST") != 0) 
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_CAMERA_SCL);
   // return -EIO;
  }  
 #endif 
#if 1
  ...
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_VGA_STBY);
    //return -EIO;
  }
#endif
  ...
  {
    printk(S5K5BBGX_MOD_NAME "Could not request GPIO %d", OMAP3430_GPIO_VGA_RST);
   // return -EIO;
  }        
  ...
  udelay(200);
   mdelay(10);
  /*3 Clock Enable */
  isp_set_xclk(0,S5K5BBGX_XCLK,0);
  mdelay(10);
...
static int s5k5bbgx_disable_gpio(void)
{

  printk(S5K5BBGX_MOD_NAME "s5k5bbgx_disable_gpio is called...\n");
 ...
 /*1 VGA_RST */
  ...
  /*5 2M_ENB LOW */
  gpio_direction_output(OMAP3430_GPIO_CAMERA_STBY, 0);
  gpio_direction_output(OMAP3430_GPIO_VGA_STBY, 0);	// JinoKang	Add
  udelay(100);
...
static int s5k5bbgx_sensor_power_set(enum v4l2_power power)
{
  static enum v4l2_power c_previous_pwr = V4L2_POWER_OFF;

  int err = 0;

  printk("YGLEE s5k5bbgx_sensor_power_set is called...[%x]\n", power);

  switch (power) 
  {
    case V4L2_POWER_OFF:
    {
      err = s5k5bbgx_disable_gpio();
    }
    break;

    case V4L2_POWER_ON:
    {
      isp_configure_interface(0,&s5k5bbgx_if_config);

      err = s5k5bbgx_enable_gpio();    
	if(err)
	{
	       printk("s5k5bbgx_sensor_power_set RETRY...[%x]\n", err);	
	       err = s5k5bbgx_disable_gpio();	
		err = s5k5bbgx_enable_gpio();    	
	}			  
    }
...
static int s5k5bbgx_ifparm(struct v4l2_ifparm *p)
{
  printk("s5k5bbgx_ifparm is called...\n");
...
static int s5k5bbgx_sensor_set_prv_data(void *priv)
{
  struct omap34xxcam_hw_config *hwc = priv;

  printk("s5k5bbgx_sensor_set_prv_data is called...YGLEE\n");

//  hwc->u.sensor.xclk = s5k5bbgx_hwc.xclk;
  hwc->u.sensor.sensor_isp = s5k5bbgx_hwc.sensor_isp;
  hwc->u.sensor.capture_mem = s5k5bbgx_hwc.capture_mem;
  hwc->dev_index =  0; 
  hwc->dev_minor =  0; 
...
struct s5k5bbgx_platform_data nowplus_s5k5bbgx_platform_data = {
  .power_set      = s5k5bbgx_sensor_power_set,
  .priv_data_set  = s5k5bbgx_sensor_set_prv_data,
  .ifparm         = s5k5bbgx_ifparm,
};
