...
int temp_dss_suspend_flag;
extern int  dss_resume_complete;
...
static void __enable_isp_rsz(struct omap_vout_device *vout, int line)
{
	...
	//The Below condition has been modified to use DSS resizer for video resolution below 800x600. CSR OMAPS00255912 
	if ((vout->pix.height == 720) || (vout->pix.width == 1280) || ((vout->pix.height * vout->pix.width) > (864 * 608)))
	{
		stream_on = 1;
		printk(" stream_on %d\n",stream_on  );
...
static int vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	if(temp_dss_suspend_flag)
	{
		ret = -EBUSY;
		printk("temp_dss_suspend_flag == TRUE\n"); 
		goto streamon_err;	
	}

	while(!dss_resume_complete)
	{
		printk("vidioc_streamon: dss_resume_complete = FALSE, wait for 100 ms.\n");
		mdelay(100);
	}
