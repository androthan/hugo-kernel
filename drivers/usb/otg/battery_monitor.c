...
// Prototype
extern void change_charging_current(int , bool is_sleep);
	int get_vbat_adc(bool);
...
static int get_elapsed_time_msecs( unsigned long long * );
...
extern void bq27425_exit(void);
extern int  bq27425_init(void);
extern unsigned int bq27425_get_vcell(void);
extern unsigned int bq27425_get_soc(void);
extern unsigned int bq27425_get_temperature(void);
extern unsigned int bq27425_get_FullChrgeCapacity(void);
extern unsigned int bq27425_get_RemainingCapacity(void);
extern unsigned int bq27425_get_FullAvailableCapacity(void);
extern unsigned int bq27425_get_AverageCurrent(void);
...
__ATTR_SHOW_CALLBACK( show_batt_vol_adc, get_vbat_adc(0) )
                    ...
                    const char *buf, size_t size)
{
	...
	switch (off) {
			...
			case BROWSER:
					...
					break;
			default:
					dprintk("not exist offset=%d", off);
					break;
	}
#endif
	dprintk("[BM] %s, offset=%d, value=%d, evt:%x",__func__, off, flag, event_logging);
    ...
    if( category == STATUS_CATEGORY_CABLE )
    {
        switch( value )
        {
            ...
            case POWER_SUPPLY_TYPE_MAINS :
                sec_bci.charger.charging_timeout = DEFAULT_CHARGING_TIMEOUT;
		/* this function should not be called during suspend. */
                if(!is_sleep)
...
static int get_elapsed_time_msecs( unsigned long long *start )
{
    unsigned long long now;
    unsigned long long diff;
    unsigned long long max = 0xFFFFFFFF;
    ...
    do_div(diff, 1000000L);
	printk("[get_elapsed_time_msecs] diff : %d \n",(int)diff);
    return (int)diff;       
}

int check_vf_gaptime()
{

 static unsigned long long vf_start;
 static int first_vf =0;
 if (first_vf == 0)
 	{
 	first_vf++;
	printk("[check_vf_gaptime] first_vf  : %d \n", first_vf);
 	vf_start = sched_clock();
 	return 300;
 	}
return get_elapsed_time_msecs( &vf_start );
...
static int get_battery_level_adc( bool flag )
{
	...
	if (!flag) {
		#ifdef BQ27425_FUEL_GAUGE

		value = bq27425_get_vcell();
...
static int get_battery_level_ptg( void )
{
	...
	fcc =  bq27425_get_FullChrgeCapacity();
 	rm = bq27425_get_RemainingCapacity();
	value = (rm*100)/fcc;
	#else	
	...
	if(boot_count >5)
	{
	prev_soc = sec_bci.battery.battery_level_ptg;
	if((sec_bci.charger.is_charging == 0) && (prev_soc < value))
		value = prev_soc;
	}
	if (boot_count < 6)
	boot_count ++;
...
static int check_full_charge_using_chg_current3( int avg_charging_current )
{
	static int current_flag = 0;

    if ((sec_bci.charger.is_charging == 0) ||( sec_bci.battery.battery_level_ptg != 100) )
	{
	current_flag = 0;
	return current_flag;
    	}
	
    if ( sec_bci.battery.battery_level_vol < 4150 )
    {
        sec_bci.battery.confirm_full_by_current = 0;
        return 0;
    }
    ...
    if (current_flag == 0)
    	{
       	 if ( avg_charging_current <= CHARGE_FULL_CURRENT)
       	 {
            	sec_bci.battery.confirm_full_by_current++;
    ...
    if (current_flag == 1)
	 {
      	  	if ( avg_charging_current <= CHARGE_CUT_OFF_CURRENT)
       	 {
          	  sec_bci.battery.confirm_full_by_current++;
                   ...
                   int *bq_avg_current )
{
	...
	*bq_avg_current =  bq27425_get_AverageCurrent();
	dprintk("[BM] bq_avg_current : %d  in fleeting \n", *bq_avg_current);
	...
	{
	prev_soc = sec_bci.battery.battery_level_ptg;
	if((sec_bci.charger.is_charging == 0) && (prev_soc < ptg_val))
	ptg_val = prev_soc;
	}
	if (boot_count2 < 6)
	boot_count2 ++;
...
#ifdef BQ27425_FUEL_GAUGE
    *battery_level_vol = bq27425_get_vcell();
#else
...
//#ifdef FCHG_DBG
	dprintk( "[BM-S] (avg_vol:%d, vol:%d) (avg_vol_adc:%d, vol_adc:%d)\n",
					avr_vol,
					*battery_level_vol,
					avr_vol_adc,
					val);
	dprintk( "[BM-S] (avg_temp:%d, temp:%d) (avg_temp_adc:%d, temp_adc:%d)\n",
					avr_temp,
					*battery_temp,
					avr_temp_adc,
					temp_adc);

	dprintk("[BM-S] battery_level_ptg : %d  \n",ptg_val); 
//#endif
}
    ...
    if ( sec_bci.battery.monitor_field_temp )
    {
        ...
        if ( sec_bci.battery.support_monitor_temp )
        {
            ...
            else
            {
                ...
                {
                    if ( (sec_bci.battery.battery_health != POWER_SUPPLY_HEALTH_GOOD) && (sec_bci.battery.battery_health != POWER_SUPPLY_HEALTH_OVERVOLTAGE) &&  (sec_bci.battery.battery_health != POWER_SUPPLY_HEALTH_DEAD))
                    {
                        sec_bci.battery.battery_health = POWER_SUPPLY_HEALTH_GOOD;
                        _battery_state_change_( STATUS_CATEGORY_TEMP, 
                                    BATTERY_TEMPERATURE_NORMAL, 
...
static void battery_monitor_work_handler( struct work_struct *work )
{
	...
	int bq_average_current = 0;
	
    struct battery_device_info *di = container_of( work,
                            struct battery_device_info,
                            battery_monitor_work.work );

	/* prevent work handler from working under suspend state */
	 if(batt_gptimer_12.active)
 	{
 		dprintk("gptimer12 is already working.");
 		return;
 	}
    ...
    sec_bci.battery.battery_level_ptg = get_battery_level_ptg();
		bq_average_current =  bq27425_get_AverageCurrent();

	if(sec_bci.charger.is_charging && (sec_bci.charger.cable_status == POWER_SUPPLY_TYPE_MAINS)  )
	change_charging_current(bq_average_current,0);

	dprintk("%s: cable_status:%d, charge_status:%d rechg_vol:%d rechg_cnt:%d is_charging:%d, avr_vol:%d, cur_vol:%d avg_current : %d\n",
					__func__,
	...
	charge_current_flag = check_full_charge_using_chg_current3( bq_average_current );
...
//		bq_average_current =  bq27425_get_AverageCurrent();
//	printk("[BM] bq_avg_current : %d  in fleeting \n", bq_average_current);

	charge_current_flag = check_full_charge_using_chg_current3( bq_average_current );
	if(sec_bci.charger.is_charging && (sec_bci.charger.cable_status == POWER_SUPPLY_TYPE_MAINS) )
	change_charging_current(bq_average_current,1);
...
static int __devinit battery_probe( struct platform_device *pdev )
{
	...
	{
		ret = sysfs_create_file( &di->sec_battery.dev->kobj, &batt_sysfs_testmode[i].attr );
		if ( ret )
		{
                            ...
                            pm_message_t state )
{
	...
	dprintk("%s: cable_status:%d, charge_status:%d rechg_vol:%d rechg_cnt:%d is_charging:%d, avr_vol:%d, cur_vol:%d\n",
					__func__,
					sec_bci.charger.cable_status,
					sec_bci.charger.charge_status,
					sec_bci.battery.monitor_field_rechg_vol,
	...
	cancel_delayed_work( &di->battery_monitor_work );
	/* make sure that battery_monitor_work_handler does not work any more after suspend. */
	flush_workqueue(sec_bci.sec_battery_workq);

	sec_bci.charger.rechg_count = 0;
	if(( sec_bci.charger.cable_status == POWER_SUPPLY_TYPE_MAINS)  || (sec_bci.charger.cable_status == POWER_SUPPLY_TYPE_USB))
		request_gptimer12( &batt_gptimer_12 );
