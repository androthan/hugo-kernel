...
static int __init twl4030_configure_resource(struct twl4030_resconfig *rconfig)
{
	...
	if( (rconfig_addr + DEV_GRP_OFFSET) == 0x33);      //Don't init VPLL2
	else
	{
	/* Set resource group */
...
#if 0
	if (get_hw_revision() >0x8) {
		printk("twl4030 :: Turn on VMMC2 for sensor\n");
		err = twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x00, 0x2b);
		if(err) {
			pr_err("TWL4030 failed to write VMMC2 register!.\n");
		}
		err = twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x9, 0x2e);
		if(err) {
			pr_err("TWL4030 failed to write VMMC2 register2e!.\n");
		}
		twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0xe0, 0x2b);
		if(err) {
			pr_err("TWL4030 failed to write VMMC2 register2b!.\n");
		}
	} else {
		printk("twl4030 :: Turn off VMMC2 for TSP\n");
		err = twl_i2c_write_u8(TWL4030_MODULE_PM_RECEIVER, 0x00, 0x2b);
		if(err) {
			pr_err("TWL4030 failed to write VMMC2 register!.\n");
		}
		else {
			twl_i2c_read_u8(TWL4030_MODULE_PM_RECEIVER, &reg_val2, 0x2b);
			pr_err("TWL4030 success to write VMMC2 register(= 0x%02x)!.\n", reg_val2);
		}
	}
#endif
