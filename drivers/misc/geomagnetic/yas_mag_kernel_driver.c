	...
	int sigs)
{
	...
	sleeper->timer._expires = ktime_set(0, msecs*NSEC_PER_MSEC);
  ...
  */
 void sensor_msleep(unsigned int msecs)
 {
...
unsigned long sensor_msleep_interruptible(unsigned int msecs)
{
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (hwdep_driver.set_position != NULL) {
        if (hwdep_driver.set_position(CONFIG_INPUT_YAS_MAGNETOMETER_POSITION_411) < 0) {
