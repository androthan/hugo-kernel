...
#ifdef CONFIG_INPUT_YAS_MAGNETOMETER_411
...
#define IDEAL_X 0
#define IDEAL_Y 0
#define IDEAL_Z 1024  //111220_HUGO_HJLIM
//#define CAL_DIV 8
...
void mpu_accel_enable_set(int enable)
{
	...
	if(enable) {
		mpu->mpu_accel.is_activated = !(mldl_cfg->inv_mpu_state->status & MPU_ACCEL_IS_SUSPENDED);
		...
		if (!mpu->mpu_accel.is_activated && mpu->mpu_accel.turned_by_mpu_accel) {
...
int read_accel_raw_xyz(struct acc_data *acc)
{
	...
	retval = inv_serial_read(slave_adapter[EXT_SLAVE_TYPE_ACCEL], 0x68, 0x3B, 6, data);
...
static int accel_do_calibrate(int enable)
{
	...
	if (enable) {
		cal_data.x = ((sum[0] / CALIBRATION_DATA_AMOUNT)- IDEAL_X)*CAL_DIV;
		cal_data.y = ((sum[1] / CALIBRATION_DATA_AMOUNT)- IDEAL_Y)*CAL_DIV;
		cal_data.z = ((sum[2] / CALIBRATION_DATA_AMOUNT)- IDEAL_Z)*CAL_DIV;
	} else {
		cal_data.x = 0;
		cal_data.y = 0;
		cal_data.z = 0;
		...
		struct device_attribute *attr, char *buf)
{
	...
	inv_serial_read(slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE], DEFAULT_MPU_SLAVEADDR, 
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	ret = gpio_request(OMAP_GPIO_SENSOR_EN_1 , "SENSOR_LDO_1");
	if (ret < 0) {
	   printk("fail to get gpio : %d, res : %d\n", OMAP_GPIO_SENSOR_EN_1, ret);
	   return;
	}
        gpio_direction_output( OMAP_GPIO_SENSOR_EN_1 , 1);

	ret = gpio_request(OMAP_GPIO_SENSOR_EN , "SENSOR_LDO");
	if (ret < 0) {
	   printk("fail to get gpio : %d, res : %d\n", OMAP_GPIO_SENSOR_EN, ret);
	   return;
	}
        gpio_direction_output( OMAP_GPIO_SENSOR_EN , 1);
	mdelay(500);
...
#endif //CONFIG_INPUT_YAS_MAGNETOMETER_411
